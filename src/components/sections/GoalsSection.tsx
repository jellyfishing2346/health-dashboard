        ...
        <div className="card-content">
          ...
          <div className="flex items-center space-x-3 mb-4">
            <input
              ...
              placeholder="Add a new goal (e.g., Walk 8k steps)"
                  ...
                  <div key={g.id} className="p-4 rounded-lg border border-gray-200">
                    <div className="flex items-center justify-between">
                      <button
                        ...
                        aria-label={g.completed ? 'Mark goal as active' : 'Mark goal as complete'}
                      >
                        {g.completed ? (
                      ...
                      <div className="flex items-center space-x-2">
                        {isEditing ? (
                          <>
                            <button className="btn-secondary px-2 py-1" onClick={() => saveEdit(g)}>
                              <Save className="w-4 h-4" />
                            </button>
                            <button className="btn-ghost p-2" onClick={() => cancelEdit(g.id)}>
                              <X className="w-4 h-4" />
                            </button>
                          </>
                        ) : (
                          <button className="btn-ghost p-2" onClick={() => startEdit(g)}>
                    ...
                    <div className="mt-3">
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div
                          className={`h-2 rounded-full ${pct(g) === 100 ? 'bg-green-600' : 'bg-primary-600'}`}
                          style={{ width: `${pct(g)}%` }}
                        />
                      </div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4">
                        <div>
                          ...
                          <input
                            ...
                            value={d.targetValue ?? g.targetValue}
                            onChange={(e) => setDrafts((prev) => ({ ...prev, [g.id]: { ...prev[g.id], targetValue: Number(e.target.value) } }))}
                          />
                        </div>
                        <div>
                          ...
                          <input
                            type="number"
                            className="input-field"
                            value={d.currentValue ?? g.currentValue}
                            onChange={(e) => setDrafts((prev) => ({ ...prev, [g.id]: { ...prev[g.id], currentValue: Number(e.target.value) } }))}
                          />
                        </div>
                        <div>
                          ...
                          <input
                            ...
                            className="input-field"
                            value={d.unit ?? g.unit}
                            onChange={(e) => setDrafts((prev) => ({ ...prev, [g.id]: { ...prev[g.id], unit: e.target.value } }))}
                          />
                        </div>
                        <div>
                          ...
                          <input
                            type="date"
                            className="input-field"
                            value={(() => { const dt = d.targetDate ?? g.targetDate; return dt ? new Date(dt as any).toISOString().split('T')[0] : ''; })()}
                            onChange={(e) => setDrafts((prev) => ({ ...prev, [g.id]: { ...prev[g.id], targetDate: e.target.value as any } }))}
                          />
                        </div>
                      </div>
                    )}
