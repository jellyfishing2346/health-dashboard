...
import { exportMentalCSV } from '@/utils/csv';

interface MentalSectionProps { data: MentalHealthData[] }
...
export const MentalSection: React.FC<MentalSectionProps> = ({ data }) => {
  ...
  const filterByMetrics = (items: MentalHealthData[]) => items.filter(x => {
        ...
        <div className="card-header">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <div className="w-10 h-10 bg-purple-100 rounded-lg flex items-center justify-center"><Brain className="h-5 w-5 text-purple-600" /></div>
              <div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {range==='custom' && (
                <div className="flex items-center gap-2">
                  ...
                  <input id="ment-start" type="date" className="input-field" value={startDate} onChange={e=>setStartDate(e.target.value)} />
                  <label className="text-sm text-gray-600" htmlFor="ment-end">To</label>
                  <input id="ment-end" type="date" className="input-field" value={endDate} onChange={e=>setEndDate(e.target.value)} />
                </div>
              )}
              {/* Export CSV */}
              <button
                onClick={() => exportMentalCSV(filtered)}
          ...
          <div className="mt-3 flex flex-wrap items-center gap-4 text-sm">
            <label className="inline-flex items-center gap-2 cursor-pointer"><input type="checkbox" className="w-4 h-4" checked={fMood} onChange={e=>setFMood(e.target.checked)} /><span className="text-gray-600">With Mood</span></label>
            <label className="inline-flex items-center gap-2 cursor-pointer"><input type="checkbox" className="w-4 h-4" checked={fStress} onChange={e=>setFStress(e.target.checked)} /><span className="text-gray-600">With Stress</span></label>
            <label className="inline-flex items-center gap-2 cursor-pointer"><input type="checkbox" className="w-4 h-4" checked={fMeditation} onChange={e=>setFMeditation(e.target.checked)} /><span className="text-gray-600">With Meditation</span></label>
          </div>
        </div>
                ...
                <div key={entry.id} className="p-4 rounded-lg border border-gray-200">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {entry.journalingDone ? (
                      <span className="text-xs font-medium text-indigo-700 bg-indigo-100 rounded-full px-2 py-0.5">Journaled</span>
                    ) : null}
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="text-center bg-gray-50 rounded p-3"><p className="text-xs text-gray-600">Mood</p><p className="text-lg font-semibold text-gray-900">{entry.mood ?? '\u2014'}{entry.mood ? '/5' : ''}</p></div>
                    <div className="text-center bg-gray-50 rounded p-3"><p className="text-xs text-gray-600">Stress</p><p className="text-lg font-semibold text-gray-900">{entry.stressLevel ?? '\u2014'}{entry.stressLevel ? '/5' : ''}</p></div>
                    <div className="text-center bg-gray-50 rounded p-3"><p className="text-xs text-gray-600">Energy</p><p className="text-lg font-semibold text-gray-900">{entry.energyLevel ?? '\u2014'}{entry.energyLevel ? '/5' : ''}</p></div>
                    <div className="text-center bg-gray-50 rounded p-3"><p className="text-xs text-gray-600">Meditation</p><p className="text-lg font-semibold text-gray-900">{entry.meditationMinutes ?? '\u2014'}{entry.meditationMinutes ? ' min' : ''}</p></div>
                  </div>
